  ...
  return (
    <div className="space-y-3 animate-pop">
      ...
      <div className="flex gap-1.5">
        {kinds.map((item) => (
          <button key={item.id} onClick={() => setKind(item.id)} className={`press flex-1 rounded-[16px] px-3 py-2.5 text-center text-[11.5px] font-bold transition-all ${kind === item.id ? "bg-amber text-white shadow-sm" : "border border-ink/18 bg-paper2 text-ink2"}`}>
            <span className="inline-flex items-center justify-center gap-1.5"><IconSymbol symbol={item.icon} size={16} /> {item.label}</span>
          </button>
        ))}
      </div>
      <input value={title} maxLength={120} onChange={(event) => setTitle(event.target.value)} placeholder={t("pushTitlePh")} className="w-full rounded-[16px] border border-ink/15 bg-card px-4 py-3 text-[13.5px] font-semibold text-ink outline-none placeholder:text-ink2/75 focus:border-moss" />
      <textarea value={body} maxLength={1000} onChange={(event) => setBody(event.target.value)} placeholder={t("pushBodyPh")} rows={4} className="w-full resize-none rounded-[16px] border border-ink/15 bg-card px-4 py-3 text-[13.5px] font-semibold text-ink outline-none placeholder:text-ink2/75 focus:border-moss" />
      <button onClick={() => void submit()} disabled={!title.trim() || !body.trim() || sending} className="press flex h-12 w-full items-center justify-center gap-2 rounded-[18px] bg-amber text-[13.5px] font-bold text-white shadow-sm disabled:opacity-40">
